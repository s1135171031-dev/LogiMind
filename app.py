...
import time# ==========================================
# 檔案名稱: app.py
# 用途: CityOS 主程式 (V19.0 Ultimate Edition)
# 功能: 每日快報、怪誕任務、隱藏成就觸發、CLI 駭客系統
# ==========================================
...
# 注意：這裡引入了 HIDDEN_MISSIONS 以便在介面上顯示成就
...
def page_dashboard(uid, user):
    ...
    with st.container(border=True):
        ...
        with col_icon:
            st.markdown(f"<div style='font-size: 50px; text-align: center; padding-top: 10px;'>{icon}</div>", unsafe_allow_html=True)
        with col_text:
    ...
    with tab1:
        st.subheader("📡 即時數據監控")
        run_monitor = st.checkbox("🔴 啟動數據串流 (Live Stream)")
    ...
    with tab3:
        ...
        ### 📘 操作指南
        **1. 隱藏要素 (Easter Eggs)**
        * 嘗試奇怪的行為：例如把錢花光、存到特定數字。
        * 在 CLI 輸入錯誤指令，或尋找管理員密碼。
...
def page_missions(uid, user):
    ...
    if not missions:
        st.error("❌ 無法讀取 missions.txt，請執行 gen_missions.py。")
        return
    ...
    st.progress(progress, text=f"一般任務進度: {len(valid_done)}/{total}")
    ...
    with tab_n:
        ...
        with col1:
            ...
            for mid, m in missions.items():
                if mid not in done and count < 8: # 限制顯示數量
                    with st.container(border=True):
                        st.write(f"**{m['title']}**")
                        st.caption(m['desc'])
                        st.write(f"💰 報酬: ${m['reward']}")
                    count += 1
            if count == 8: st.caption("... 完成上方任務以顯示更多")
    ...
    with tab_h:
        ...
        if not hidden_done:
            st.info("尚無隱藏成就。提示：嘗試讓錢歸零、或者在實驗室亂按開關。")
        else:
...
def page_digital_lab(uid, user):
    ...
    with t1:
        ...
        # 🔥 隱藏任務觸發點：H_LOGIC (電路過載)
        if a and b:
            check_mission(uid, user, "logic_state", extra_data="11")
        elif g and (a or b):
...
def page_cli_os(uid, user):
    ...
    if cmd:
        ...
        if t[0] not in valid_cmds:
            ...
            st.session_state.cli_err_cnt = 0 # 重置錯誤計數
...
def main():
    ...
    if not st.session_state.logged_in:
        st.markdown("<h1 style='text-align: center;'>🏙️ CityOS V19.0</h1>", unsafe_allow_html=True)
        ...
        with t2:
            ...
            if st.button("註冊"):
                ...
                if nu not in db["users"]:
                    ...
                    save_db(db); st.success("註冊成功！請切換至登入頁籤。")
                else: st.error("帳號已存在")
        return
