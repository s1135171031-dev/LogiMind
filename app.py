...
def apply_theme(p):
    ...
    /* 【V41 新增】圖片容器專屬樣式：強制白底卡片化 */
    div[data-testid="stImage"] {{
        background-color: #FFFFFF !important; /* 強制白底 */
        padding: 20px !important;             /* 增加留白 */
        border-radius: 15px !important;       /* 圓角效果 */
        box-shadow: 0 4px 6px rgba(0,0,0,0.1); /* 輕微陰影提升質感 */
        display: flex;
...
def main():
    ...
    # --- 2. 基礎邏輯閘 (含圖片與真值表) ---
        ...
        with col1:
            ...
            # 這裡的圖片現在會有漂亮的白底卡片效果
            st.image(GATE_IMGS[g_name], use_column_width=True)
