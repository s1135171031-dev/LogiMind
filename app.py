...
def init_user_db():
    """檢查 users.json，如果是空的或不存在，就填入預設資料 (包含 Frank)"""
    should_init = False
...
if "user_data" not in st.session_state:
    ...
    st.session_state.update({
        "logged_in": False, 
        "user_key": "", # 用來記錄目前登入的是哪個帳號 ID (例如 'frank')
        "user_data": {}, 
...
def render_svg(svg_code):
    # 根據當前主題簡單調整顏色，這裡強制轉黑或白比較複雜，先維持原邏輯(轉黑)
    # 若要更細緻，可根據 theme 調整
    ...
    if "城市儀表板" in page:
        ...
        with col2:
            ...
            if os.path.exists("questions.txt"):
                ...
                st.error("❌ Questions.txt (Missing)")
        ...
        st.header("🏦 運算中心")
        val = st.text_input("輸入數值 (Decimal)", "127")
        if val.isdigit(): 
            st.metric("Hex", hex(int(val))[2:].upper())
            st.metric("Binary", bin(int(val))[2:])
        ...
        if not st.session_state.exam_active:
            if st.button("🚀 啟動考核"):
                ...
                if len(qs) >= 5:
                    ...
                    st.rerun()
                else: st.error("題庫檔案 (questions.txt) 不足或遺失！請先建立題庫。")
        else:
...
def login_page():
    ...
    with col2:
        ...
        if not os.path.exists("questions.txt"):
            st.warning("⚠️ 警告：題庫 questions.txt 遺失。請建立該檔案以使用考試功能。")
