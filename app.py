...
def update_stock_market():
    ...
    if now - global_state.get("last_update", 0) > 0.5:
        ...
        for code, data in STOCKS_DATA.items():
            ...
            jitter = random.randint(1, 3) * direction
...
def page_mining(uid, user):
    ...
    c1.metric("Hash", st.session_state.mined_hashes)
    c2.metric("GPU 溫度", f"{st.session_state.mining_temp:.1f}°C", delta_color="inverse", delta=f"{st.session_state.mining_temp-40:.1f}")
    ...
    with col_a:
        if st.button("⛏️ 運算 (MINE)", use_container_width=True):
            ...
            if st.session_state.mining_temp >= 100:
                ...
                user['toxicity'] += dmg
                st.session_state.mining_temp = 60.0
                st.session_state.mined_hashes = 0
                save_user(uid, user)
                add_log(f"🔥 {uid} 的礦機爆炸了！")
                st.toast(f"爆炸! HP -{dmg}", icon="🔥")
                time.sleep(1)
                st.rerun()
    with col_b:
        if st.button("❄️ 散熱 (COOL)", use_container_width=True):
            st.session_state.mining_temp = max(40.0, st.session_state.mining_temp - 15.0)
            st.rerun()
    st.session_state.mining_temp = max(40.0, st.session_state.mining_temp - 0.5)
    st.divider()
    val = st.session_state.mined_hashes * 2
    if st.button(f"💰 兌現 (+${val})"):
        if val > 0:
            user['money'] += val
            st.session_state.mined_hashes = 0
            save_user(uid, user)
...
def page_casino(uid, user):
    st.title("🎰 霓虹賭場")
    bet = st.number_input("下注", 10, max(10, user['money']), 100)
    c1, c2 = st.columns(2)
    with c1:
        if st.button("賭大小 (x2)"):
            if user['money']>=bet:
...
def page_stock(uid, user):
    ...
    with c2:
        sel = st.selectbox("標的", list(STOCKS_DATA.keys()))
        curr = prices.get(sel, 0)
        st.metric(f"現價 {sel}", f"${curr}")
        t1, t2 = st.tabs(["買", "賣"])
        with t1:
            q = st.number_input("買量", 1, 1000, 10, key="bq")
            cost = curr * q
        ...
        with t2:
            own = user.get('stocks',{}).get(sel,0)
            st.write(f"持有: {own}")
            sq = st.number_input("賣量", 1, max(1,own), 1, key="sq")
            inc = curr * sq
        ...
        for k, v in inv.items():
            if v > 0:
                ...
                if k == "Anti-Rad Pill":
                    if c2.button("💊 吃"):
                        user["inventory"][k]-=1
                        user["inventory"].update({x:0 for x in ["Nutri-Paste","Stim-Pack","Cyber-Arm"] if user["inventory"].get(x,0)>0})
                        user["toxicity"]=0
                        save_user(uid,user)
                        st.success("解毒")
                        st.rerun()
...
def page_crypto(uid, user):
    ...
    with t1:
        ...
        pt = st.text_area("文", "ATTACK")
        st.code("".join([chr((ord(c)-65+s)%26+65) if c.isupper() else c for c in pt]))
    with t2:
        txt = st.text_input("B64", "Hello")
        st.code(base64.b64encode(txt.encode()).decode())
    with t3:
        if "caesar_ans" not in st.session_state:
            w = random.choice(["LINUX", "JAVA", "PYTHON"])
            k = random.randint(1,5)
            st.session_state.caesar_target = w
            st.session_state.caesar_shift = k
            st.session_state.caesar_q = "".join([chr(ord(c)+k) for c in w])
        st.write(f"密: **{st.session_state.caesar_q}**")
        ans = st.text_input("答")
        if st.button("驗"): 
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        with c1: 
            ...
            if st.button("連線"): 
                if get_user(u) and get_user(u)['password']==p:
                    st.session_state.logged_in=True
                    st.session_state.uid=u
                    st.rerun()
        with c2:
            nu=st.text_input("新ID")
            np=st.text_input("新PW", type="password")
            nn=st.text_input("名")
            if st.button("建立"): 
    ...
    with st.sidebar:
        ...
        st.caption(f"Lv.{user['level']} {LEVEL_TITLES.get(user['level'],'')}")
        st.progress(min(1.0, user['exp']/(user['level']*100)))
        st.metric("Money", f"${user['money']}")
        st.metric("Tox", f"{user['toxicity']}%")
        nav = st.radio("Menu", ["Dashboard", "Exchange", "Mining Farm", "Dark Market", "Casino", "PVP", "Logic Gates", "Crypto", "Linux"])
        st.divider()
        st.subheader("📡 Public Net")
