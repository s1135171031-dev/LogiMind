...
def get_npc_data(name, job, level, money):
    return {
        ...
        "completed_missions": [], "pending_claims": []
    }
    ...
    if not os.path.exists(USER_DB_FILE):
        users = {
            ...
            "frank": {
                ...
                "job": "Architect", "inventory": {"Mining GPU": 99, "Firewall": 100}, 
                "completed_missions": [], "pending_claims": []
            }
        }
        ...
        # 遷移邏輯：確保 defense_code 和 pending_claims 存在
        try:
...
def log_intruder(username):
    with open(LOG_FILE, "a", encoding="utf-8") as f:
        f.write(f"[{datetime.now()}] Failed: {username}\n")
...
def check_mission(uid, user, action_type, extra_data=None):
    ...
    def _trigger(mid, title):
        nonlocal triggered_new
        # 只有當「沒領過」且「不在待領清單」時才觸發
        if mid not in user["completed_missions"] and mid not in user["pending_claims"]:
            user["pending_claims"].append(mid)
            st.toast(f"🚩 任務達成：【{title}】！請至任務中心領獎。", icon="🎁")
            triggered_new = True

    # 1. 普通任務
    for mid, m_data in missions.items():
        if m_data["target"] == action_type:
            _trigger(mid, m_data['title'])

    # 2. 隱藏成就條件
    if user["money"] == 0: _trigger("H_ZERO", HIDDEN_MISSIONS["H_ZERO"]["title"])
    if user["money"] == 777: _trigger("H_777", HIDDEN_MISSIONS["H_777"]["title"])
    if sum(user.get("inventory", {}).values()) >= 15: _trigger("H_SHOP", HIDDEN_MISSIONS["H_SHOP"]["title"])
    if user.get("bank_deposit",0) > 100000 and user["money"] < 100: _trigger("H_BANK", HIDDEN_MISSIONS["H_BANK"]["title"])
    
    if action_type == "cli_input" and extra_data == "sudo su": _trigger("H_HACK", HIDDEN_MISSIONS["H_HACK"]["title"])
    if action_type == "crypto_input" and str(extra_data) == "1024": _trigger("H_MATH", HIDDEN_MISSIONS["H_MATH"]["title"])
    if action_type == "logic_state" and extra_data == "11": _trigger("H_LOGIC", HIDDEN_MISSIONS["H_LOGIC"]["title"])
    if action_type == "pvp_win": _trigger("H_PVP_W", HIDDEN_MISSIONS["H_PVP_W"]["title"])
    if action_type == "cli_error" and isinstance(extra_data, int) and extra_data >= 5: _trigger("H_SPAM", HIDDEN_MISSIONS["H_SPAM"]["title"])

    # 3. 存檔
    if triggered_new and uid != "frank":
